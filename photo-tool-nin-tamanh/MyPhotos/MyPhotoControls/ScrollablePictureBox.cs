...
namespace Manning.MyPhotoControls
{
    [DefaultEvent("Click")]
        ...
        [ToolboxBitmap(typeof(ScrollablePictureBox), "crollablePictureBox.bmp")]
    public class ScrollablePictureBox : PictureBox
    {
        private bool _allowScrollaBars = true;
        [Browsable(true)]
        [Category("Layout")]
        [DefaultValue(true)]
        [Description("Gets or sets whether the control can display scroll bars")]
        public bool AllowScollaBars
        {
            get { return _allowScrollaBars; }
            set
            {
                if (_allowScrollaBars != value)
                {
                    _allowScrollaBars = value;
                    Invalidate();
                }
            }
        }
        ...
        {
            // Initialize horizontal sroll bar
            HBar.Visible = false;
        ...
        {
            if (Image == null || AllowScollaBars == false
